 ...
 * 
 * Вывода значения угла в градусах от нескольких датчиков AS5601 в "Монитор порта"
 ...
 * Объект должен быть один, т.к. адреса датчиков не изменяются
...
void setup() {
   ...
   * В даном случае действия выполняются через Wire,
   * а не через Encoder, т.к. шина общая для всех устройств,
   * но можно Encoder.setClock, т.к. это метод вызывает Wire.setClock
   */
  Wire.begin();
  Wire.setClock(AS5601_I2C_CLOCK_400KHZ);
...
void loop() {
  ...
  Serial.println(Encoder.getDegreesAngle());
